...
import {
  GenerateAdmissionInvoiceDocument,
...
function InvoiceCard({
  ...
  const [recordPayment, { loading: isPaying }] = useMutation(RecordStudentPaymentNewDocument);
  ...
  const invoiceType = invoice.invoiceType as InvoiceType | undefined;
        ...
        <Stack spacing={0.4}>
          <Stack direction="row" spacing={1} alignItems="center">
            {invoiceType === "ADMISSION" ? (
              <HowToRegRounded sx={{ fontSize: 16, color: "primary.main" }} />
            ) : (
              <RepeatRounded sx={{ fontSize: 16, color: "secondary.main" }} />
            )}
            <Typography variant="body2" fontWeight={700}>
              {invoiceType ? INVOICE_TYPE_LABEL[invoiceType] : "Invoice"} — {dayjs(invoice.month, "YYYY-MM").format("MMMM YYYY")}
            </Typography>
          </Stack>
...
export function AdmissionInvoiceDialog({
  ...
  const [generateAdmissionInvoice, { loading: isGenerating }] = useMutation(
    GenerateAdmissionInvoiceDocument,
  );
  ...
  // Separate admission vs monthly invoices for this batch
  const admissionInvoices = batchInvoices.filter((inv) => inv.invoiceType === "ADMISSION");
  const monthlyInvoices = batchInvoices.filter((inv) => inv.invoiceType === "MONTHLY_FEE");
  ...
  const handleGenerate = async () => {
    try {
      await generateAdmissionInvoice({
        variables: { studentId, batchId, enrollmentId },
      });
    ...
    >
      ...
      <DialogContent dividers sx={{ p: 0 }}>
        {invoicesLoading && !hasAnyInvoice ? (
          ...
          <Stack>
            {/* No invoice yet — show fee summary + generate button */}
            {!hasAnyInvoice && (
              <Box sx={{ px: 3, py: 3 }}>
                {hasFees ? (
                  <Stack spacing={2.5}>
                    <Alert severity="info" icon={<WarningAmberRounded fontSize="small" />}>
                      No invoice generated yet. Review the fees below and generate the admission invoice.
                    </Alert>
                    ...
                    <Button
                      ...
                      fullWidth
                      startIcon={isGenerating ? <CircularProgress size={16} color="inherit" /> : <HowToRegRounded />}
                    >
                      {isGenerating ? "Generating…" : "Generate Admission Invoice"}
                    </Button>
                  </Stack>
                ) : (
                  <Alert severity="warning">
                    No fees configured for this batch. Add fee types to the batch before generating an invoice.
                  </Alert>
                )}
              </Box>
            )}
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        <Button color="inherit" onClick={onClose} disabled={isGenerating}>
